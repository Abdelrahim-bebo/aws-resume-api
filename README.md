...
## 📊 Monitoring, Observability & Health Optimization
The project transitioned from basic infrastructure to a "Production-Ready" state. The focus was on establishing full visibility into the system's health and automating notifications for infrastructure lifecycle events.
---

### 1. 🏥 Application Refactoring & Health Check Alignment
To move away from temporary workarounds, the application was refactored to support professional health monitoring.

* **Professional UI:** Replaced the plain-text response with a Bootstrap-based landing page to provide a visual status of all AWS components (S3, EFS, ECR, ALB).
* **Dedicated Health Endpoint:** Implemented a `/health` route returning a JSON status, allowing the ALB to perform deep health checks.
* **ALB Optimization:** Reconfigured the **Target Group** to strictly require an **HTTP 200 OK** status, ensuring only fully operational instances receive traffic.

> **Architect Insight:** Standardizing health checks to HTTP 200 is a critical best practice for production environments to prevent "False Positives" in monitoring.

---

### 2. 📈 Centralized Operations Dashboard (CloudWatch)
A custom **CloudWatch Dashboard** (`Resume-API-Operations`) was created to provide a "single pane of glass" view of the entire event-driven pipeline.

#### **Key Metrics Visualized:**
| Metric Name | Service | Purpose |
| :--- | :--- | :--- |
| **RequestCount** | Application Load Balancer | Tracks user engagement and traffic spikes on the landing page. |
| **NumberOfMessagesSent** | Amazon SQS | Confirms that S3 upload events are successfully reaching the queue. |
| **Invocations** | AWS Lambda | Verifies that the serverless worker is processing resumes as expected. |

---

### 3. 🚨 Proactive Alerting & Infrastructure Elasticity
Instead of manual monitoring, we implemented automated notifications to track how the infrastructure "breathes" (Scales).

#### **ASG Activity Notifications:**
We integrated the **Auto Scaling Group (ASG)** with **Amazon SNS** to alert the engineering team during lifecycle events. This replaces manual alarm editing with a more robust, event-based notification system.

* **Events Monitored:**
    * `Instance Launch`: Notifies when the system scales out due to high traffic.
    * `Instance Terminate`: Notifies during scale-in or when unhealthy instances are replaced.
    * `Failed to Launch`: Critical alert for troubleshooting capacity or configuration issues.

#### **Managed Alarms Integration:**
Analyzed the **Target Tracking Scaling Alarms** (High/Low CPU) generated by AWS to understand the logic behind the system's elasticity, ensuring a 70% CPU target is maintained.



### 🧪 Final Verification (The Chain Reaction)
The end-to-end observability flow was verified through a "Stress Test":
1.  **Trigger:** Multiple resumes were uploaded via the API.
2.  **Observation:** Real-time spikes were observed in the **RequestCount** and **SQS** widgets.
3.  **Validation:** Received an automated email via **SNS** confirming the **Lambda** invocation and task completion.
4.  **Result:** System status: **100% Operational & Observed.** ✅

---

### 🚀 Phase 4: CI/CD Automation (GitHub Actions & Amazon ECR)

#### 📖 Overview
To achieve a professional DevOps workflow, I implemented a **Continuous Integration (CI)** pipeline using **GitHub Actions**. This automation ensures that every code change is automatically built into a Docker container and stored in a secure cloud registry, eliminating manual errors and significantly reducing deployment time.

#### 🏗️ CI/CD Workflow Architecture
The pipeline follows a structured flow to move code from development to the cloud:
1.  **Code Push:** A developer pushes code to the `main` branch.
2.  **GitHub Actions Trigger:** The push triggers a workflow on a virtual Ubuntu runner.
3.  **AWS Authentication:** GitHub authenticates securely with AWS using IAM Access Keys stored in encrypted secrets.
4.  **Docker Build:** The application is built into a fresh Docker image based on the `Dockerfile`.
5.  **ECR Push:** The new image is tagged and pushed to the **Amazon Elastic Container Registry (ECR)** repository.

#### 🔐 Security & Secret Management
To maintain the **Principle of Least Privilege** and protect sensitive data, no credentials are hardcoded. I utilized **GitHub Actions Secrets** to encrypt and store:
* `AWS_ACCESS_KEY_ID`: Programmatic access key for the IAM User.
* `AWS_SECRET_ACCESS_KEY`: Secret access key for the IAM User.

#### 📄 Pipeline Configuration (`deploy.yml`)
The automation logic is defined in `.github/workflows/deploy.yml`. This file manages the environment setup and the Docker build/push process:
